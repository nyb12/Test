...
export default function SimpleChatbot({
  ...
  const [localSelectedAircraft, setLocalSelectedAircraft] =
    useState<any>(selectedAircraft);
  const [isFleetSpanMode, setIsFleetSpanMode] = useState(false);
  const [lastSelectiveActionTool, setLastSelectiveActionTool] = useState<
    string | null
  >(null);
  const [selectedGreetingTool, setSelectedGreetingTool] = useState<
    string | null
  >(null);
  ...
  const handleSelectiveActionClick = async (prompt: any) => {
    ...
    const preventDoubleClick =
      prompt.allow_double_click === false ||
      (prompt.prompt_value === 'Manage Contacts' &&
        prompt.allow_double_click === undefined);
    ...
    const eventHandler =
      selectiveActionFunctions[
        prompt.on_click_event as keyof typeof selectiveActionFunctions
      ];
    if (eventHandler) {
      ...
      try {
        ...
        const filteredAircraft = allAircraft.filter((aircraft: any) => {
          ...
          if (status === 'operational') {
            ...
            return (
              aircraft.primary_status === 'limited/monitor' ||
        ...
        if (filteredAircraft.length === 0) {
          ...
          const aircraftMessage = {
            ...
            } ${prompt.prompt_value.toLowerCase()} aircraft:`,
            isUser: false,
            isAircraftCard: true,
  ...
  const [selectedSelectiveAction, setSelectedSelectiveAction] = useState<
    string | null
  >(null);
  const [selectedToolboxTool, setSelectedToolboxTool] = useState<string | null>(
    null,
  );
    ...
    filterFleetByStatus: async (promptValue: string) => {
      ...
      try {
        ...
        const filteredAircraft = aircraftData.filter(
          (aircraft: any) =>
            aircraft.primary_status?.toLowerCase() ===
            promptValue.toLowerCase(),
        );
        ...
        const aiResponse = await fetch('/api/chat', {
          ...
          body: JSON.stringify({
            message: `Fleet status report: Found ${
              filteredAircraft.length
            } ${promptValue.toLowerCase()} aircraft. Provide operational insights.`,
    ...
    try {
      ...
      if (conversationMessages.length > 0) {
        ...
        if (userMessages.length > 0 && botMessages.length > 0) {
          ...
          for (
            let i = 0;
            i < Math.min(userMessages.length, botMessages.length);
            i++
          ) {
